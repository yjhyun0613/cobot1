...
# =================================================================
# [3] Step 2: DRL 생성기 (movec + ori=0 완벽 융합)
# =================================================================
def generate_scan_drl():
    ...
    for i in range(TOTAL_LAYERS):
        ...
        # 지그재그 방향에 따른 5개 주요 포인트 각도 (90도 간격)
        if layer_num % 2 == 1:
            angles = [180.0, 90.0, 0.0, -90.0, -180.0] # 시계방향 (CW)
        else:
            angles = [180.0, 270.0, 360.0, 450.0, 540.0] # 반시계방향 (CCW)
        ...
        for ang in angles:
            ...
            # 파이썬에서 미리 완벽하게 안쪽을 찌르는 오일러 각도 계산
        ...
        drl += f"p_start_{i} = {pts[0]}\n"
        drl += f"p_via1_{i}  = {pts[1]}\n"
        drl += f"p_mid_{i}   = {pts[2]}\n"
        drl += f"p_via2_{i}  = {pts[3]}\n"
        drl += f"p_end_{i}   = {pts[4]}\n"
        
        drl += f"movel(p_start_{i}, v=40.0, a=80.0, r=0.0)\n"
        
        # 💡 [핵심] ori=0 (자연스러운 회전)을 쓰지만, 각 점들이 이미 찌르는 방향이므로 완벽하게 스캔합니다!
        drl += f"movec(p_via1_{i}, p_mid_{i}, v=20.0, a=20.0, angle=180.0, r=10.0, ori=2)\n"
        drl += f"movec(p_via2_{i}, p_end_{i}, v=20.0, a=20.0, angle=180.0, r=10.0, ori=2)\n\n"
