...
from dsr_msgs2.msg import RobotStop # 💡 [수정] 로봇 정지 메시지 타입 임포트 추가
import DR_init
import time
import math
import numpy as np
...
class ScanRunnerNode(Node):
    ...
    def run(self):
        ...
        # 충돌 시 로봇을 보호하기 위해 Z축만 부드럽게 스프링 처리
        task_compliance_ctrl([3000, 3000, 100, 300, 300, 300])
        
        # 바닥 높이까지 15mm/s의 아주 느린 속도로 비동기(Async) 하강 지시!
        ...
        while rclpy.ok():
            ...
            # 💡 [핵심] 0.1초 동안 0.2mm 이하로 움직였다면 바닥에 닿아서 막힌 것!
            diff = abs(prev_z - curr_z)
            if diff < 0.2:
                # 💡 [수정] 직접 함수 호출 대신 정지 토픽(메시지) 퍼블리시
                stop_msg = RobotStop()
                stop_msg.stop_mode = 2 # STOP_TYPE_QUICK
                self.stop_pub.publish(stop_msg)
                
                print(f"🛑 바닥 감지 완료! 정지 명령(RobotStop) 전송됨 (Z값 변화량: {diff:.3f}mm)")
                break
                
            prev_z = curr_z
            
        release_compliance_ctrl()
        time.sleep(1.0) # 로봇이 완전히 멈추고 관성이 잡힐 때까지 대기
