...
# (측정은 파이썬이 끝냈으므로, 여기서는 오직 스캔 궤적만 생성합니다)
# =================================================================
def generate_scan_drl(center, radius, total_layers, tool_length):
    ...
    # 💡 로봇이 이미 꼭대기를 누르고 있는 상태에서 DRL이 시작되므로, 즉시 부드러운 스캔 텐션(-10N)으로 전환합니다.
...
class ScanRunnerNode(Node):
    ...
    def run(self):
        ...
        # 파이썬 DSR_ROBOT2 제어 함수 임포트
        from DSR_ROBOT2 import (
            movej, movel, task_compliance_ctrl, set_desired_force, 
            get_tool_force, get_current_posx, release_force, release_compliance_ctrl
        )
        
        # 1️⃣ [Phase 1] 파이썬 기반 최고점 탐색 (Python Direct Control)
        print("\n🚀 [Phase 1] 파이썬으로 돔 최고점 탐색 시작...")
        ...
        time.sleep(1.0) # 흔들림 대기

        # 파이썬에서 영점 측정
        zero_tz = get_tool_force(1)[2]
        
        # 순응 제어 및 힘 제어 켜기
        task_compliance_ctrl(stx=[3000, 3000, 100, 300, 300, 300], ref=1)
        set_desired_force(fd=[0.0, 0.0, -15.0, 0.0, 0.0, 0.0], dir=[0, 0, 1, 0, 0, 0], ref=1)
        
        # 💡 노이즈 필터링 0.5초 대기
        time.sleep(0.5)

        print("⏬ 스르륵 하강 중... (파이썬 감지 루프)")
        while rclpy.ok():
            rclpy.spin_once(self, timeout_sec=0.01) # 통신 유지
            current_fz = get_tool_force(1)[2]
            if abs(current_fz - zero_tz) > 8.0:
                break
            
        # 닿으면 파이썬에서 즉시 힘 제어 해제
        release_force()
        release_compliance_ctrl()
        ...
        self.state_pub.publish(Bool(data=True)) # 기록 ON
        ...
        self.state_pub.publish(Bool(data=False)) # 기록 OFF
