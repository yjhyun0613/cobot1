...
def rot_to_zyz(R):
    ...
    # 💡 [핵심 보완] 짐벌락(Gimbal Lock) 완벽 방어 로직
    if abs(val - 1.0) < 1e-6: # 수평일 때 (beta = 0)
        alpha = 0.0
        gamma = math.atan2(R[1, 0], R[0, 0])
    elif abs(val + 1.0) < 1e-6: # 완벽히 수직일 때 (beta = 180) - 1층 튕김 원인 해결!
        alpha = 0.0
        gamma = math.atan2(-R[1, 0], -R[0, 0])
    else: # 일반적인 각도
...
def get_dome_orientation(target_pos, center, tool_length=0.0):
    ...
    # 1. 툴의 Z축은 무조건 돔의 중심을 향함 (완벽히 꽂히는 자세)
    Z_t = -normal 
    
    # 2. 툴의 Y축은 원주 궤적의 접선(Tangent) 방향을 향함
    xy_vec = np.array([normal[0], normal[1], 0.0])
    xy_norm = np.linalg.norm(xy_vec)
    
    if xy_norm < 1e-6: # 돔 완전 꼭대기 정중앙
        X_t = np.array([1.0, 0.0, 0.0])
        Y_t = np.array([0.0, -1.0, 0.0])
    else:
        Y_t = np.array([-normal[1], normal[0], 0.0]) / xy_norm
        # 3. 툴의 X축은 Z축과 Y축의 수직(외적)
        X_t = np.cross(Y_t, Z_t)
        
    # 3x3 회전 행렬 생성 후 ZYZ 오일러 각도로 변환
    R = np.column_stack((X_t, Y_t, Z_t))
    rx, ry, rz = rot_to_zyz(R)
    
    # 툴 길이만큼 뒤로 후퇴한 플랜지 좌표 계산
...
def generate_scan_drl(center, radius, total_layers, tool_length):
    ...
    for i in range(1, total_layers + 1):
        ...
        # 궤적 포인트의 자세를 계산하고 오일러 각도 꼬임 방지 적용
        ro_pts = []
        for pt in pts:
    ...
    # 모든 스캔 종료 후 안전 높이로 복귀
    approach_z = FLOOR_Z + radius + 50.0
    drl_code += "release_force()\nrelease_compliance_ctrl()\n"
    # 접근 포인트도 완벽한 수직 자세로 복귀
    a_rx_raw, a_ry, a_rz_raw, a_pos = get_dome_orientation(np.array([cx, cy, approach_z]), true_center, tool_length)
    a_rx = make_continuous(last_rx, a_rx_raw)
    a_rz = make_continuous(last_rz, a_rz_raw)
...
class ScanRunnerNode(Node):
    ...
    def run(self):
        ...
        # 3️⃣ [Phase 2] 스캐닝 실행
        print("\n🚀 [Phase 2] 스캐닝 DRL 구동 시작 (3D 법선 벡터 완전 구면 좌표계)...")
