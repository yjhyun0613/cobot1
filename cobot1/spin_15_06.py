...
def generate_layer_waypoints(center, radius, total_layers):
    ...
    for i in range(1, total_layers + 1):
        ...
        # 💡 [핵심 해결 로직] Z축 수직 이동 완벽 차단! (Base 좌표계 기준)
        # Base X, Y는 100으로 부드럽게 (돔이 크면 수평으로만 벌어짐)
        # Base Z는 3000으로 꽉 잡음 (절대 밀려 올라가지 않음)
...
def main(args=None):
    ...
    try:
        ...
        print(f"\n🔄 신호 대기 중... (수직 상승 방지 모드 | 최대 확장 반경: +{MAX_RADIUS_EXPANSION}mm)")
        while rclpy.ok():
