...
RADIUS       = 50.0            
TOTAL_LAYERS = 5    
CENTER       = np.array([421.15, 74.76, 54.0]) # [X, Y, Z]

SAFE_Z_HEIGHT = 200.0  # 대기 위치의 안전 Z 높이
LIFT_HEIGHT   = 15.0   # 층간 이동 시 Z축으로 들어올릴 높이 (mm)

# 🌟 [추가됨] 뚱뚱한 돔(타원체)을 위한 최대 반경 확장량 (바닥층에서 늘어날 mm)
MAX_RADIUS_EXPANSION = 7.0  
...
def generate_dome_scan_drl(center, radius, total_layers):
    ...
    # 1. 첫 번째 층(Start) 위치 및 회전각 계산
    angle_rad_1 = math.radians(90.0 * (1.0 - 1.0/total_layers))
    curr_z_1 = cz + radius * math.sin(angle_rad_1)
    # 💡 1층은 꼭대기 부근이므로 반경 확장이 없습니다 (+ 0.0)
    curr_r_1 = radius * math.cos(angle_rad_1) + 0.0 
    ...
    drl_code += "stiff = [100, 3000, 300, 300, 300, 300]\n"
    drl_code += "task_compliance_ctrl(stiff)\n\n"

    # 3. 층별 지그재그 회전 로직 생성
    for i in range(1, total_layers + 1):
        # 🌟 [핵심 변경] 현재 층수에 비례하여 늘어날 반지름 계산 (선형 보간) 🌟
        # i=1 일때 0, i=total_layers 일때 1.0이 되는 비율을 만듭니다.
        ...
        # 💡 기본 수평 반경에 현재 층의 확장량(current_expansion)을 강제로 더해줍니다.
        ...
        # DRL 문자열 추가 (확장된 mm를 터미널에서 볼 수 있게 주석에 추가)
        drl_code += f"# Layer {i} - {direction} (Expand: +{current_expansion:.2f}mm)\n"
        
        # 이전 층에서 넘어올 때 위에서 아래로 진입하기 위한 리프트 진입점 (1층 제외)
        if i > 1:
            lift_in_xyz = start_xyz + np.array([0.0, 0.0, LIFT_HEIGHT])
            drl_code += f"p_lift_in_{i} = posx({lift_in_xyz[0]:.2f}, {lift_in_xyz[1]:.2f}, {lift_in_xyz[2]:.2f}, {sr_x:.2f}, {sr_y:.2f}, {sr_z:.2f})\n"
            drl_code += f"movel(p_lift_in_{i}, v=40, a=80, r=5.0)\n"
        ...
        # 정확한 시작 높이에 도달 후 원을 그리도록 r=0.0 설정
        drl_code += f"movel(p_start_{i}, v=40, a=80, r=0.0)\n"
        drl_code += f"movec(p_via_{i}, p_tgt_{i}, v=50, a=100, angle=360.0, ori=2)\n"

        # 원 궤적을 다 그리고 나서 Z축으로 살짝 들기 (마지막 층 제외)
        if i < total_layers:
            lift_out_xyz = start_xyz + np.array([0.0, 0.0, LIFT_HEIGHT])
            drl_code += f"p_lift_out_{i} = posx({lift_out_xyz[0]:.2f}, {lift_out_xyz[1]:.2f}, {lift_out_xyz[2]:.2f}, {sr_x:.2f}, {sr_y:.2f}, {sr_z:.2f})\n"
            drl_code += f"movel(p_lift_out_{i}, v=40, a=80, r=5.0)\n\n"
...
def main(args=None):
    ...
    try:
        ...
        print(f"\n🔄 대기 모드: 신호(02) 대기 중... (최대 확장 반경: +{MAX_RADIUS_EXPANSION}mm)")
