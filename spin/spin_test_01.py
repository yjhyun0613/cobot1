...
# =================================================================
# [2] 기하학 연산 함수 (균등 기울기 + 찌르는 방향 반전 + 손목 꼬임 방지)
# =================================================================
def rot_to_zyz(R):
...
def get_forced_tilt_orientation(angle_xy_deg, tilt_deg):
    ...
    # 💡 [핵심] 마이너스(-) 기호를 붙여서 툴이 돔 표면에 눕지 않고 안쪽을 날카롭게 찌르도록 변경!
    Z_t = np.array([
    ...
    # 손목 꼬임을 막기 위해 툴 몸체(Y축)를 항상 글로벌 Y축(북쪽) 기준에 맞춤
...
# =================================================================
# [3] Step 2: DRL 생성기 (실측 좌표 기반 36분할 블렌딩 + 순응 제어)
# =================================================================
def generate_scan_drl():
    ...
    # 💡 [핵심] 충돌 방지용 순응 제어 (Compliance Control) 켜기
    # sys_type=1 (툴 좌표계) 기준으로 Z축(찌르는 방향)의 강성(Stiffness)을 500으로 부드럽게 풉니다.
    ...
    for i in range(TOTAL_LAYERS):
        ...
        # 균등한 기울기 분배 로직 (마지막 층이 최대 각도가 되도록)
