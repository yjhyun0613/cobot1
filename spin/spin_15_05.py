...
class ScanRunnerNode(Node):
    ...
    def run(self):
        ...
        for i, layer in enumerate(layers_data):
            ...
            # 1. 띄운 상태로 이동 및 표면 안착 (wait_for_idle 완전 제거, r 값으로 스무스하게 연결!)
            if layer['lift_in']:
                # 공중 이동: 멈추지 않고 곡선으로 지나가도록 r=15.0 부여, 속도 상향
                movel(layer['lift_in'], vel=60.0, acc=100.0, r=15.0)
                # 안착 위치: 스위치를 켜야 하므로 여기서만 정확히 멈추도록 r=0.0 부여
                movel(layer['start'], vel=30.0, acc=60.0, r=0.0)

            # 2. 🟢 표면 도착 완료! 스위치 ON 및 스캔 진행
            ...
            # 💡 이 대기 시간은 DRL(movec 스캔)이 끝나는 걸 확실히 기다려야 하므로 필수 유지!
            ...
            # 4. 다음 층을 위해 수직으로 띄우기
            if layer['lift_out']:
                # 탈출 후 바로 다음 공중 이동으로 이어지게 r=15.0 부여
                movel(layer['lift_out'], vel=60.0, acc=100.0, r=15.0)
...
def main(args=None):
    ...
    try:
        ...
        print(f"\n🔄 신호 대기 중... (논스톱 모션 블렌딩 | 최대 확장 반경: +{MAX_RADIUS_EXPANSION}mm)")
        while rclpy.ok():
