...
# 💡 중복 부하를 없애기 위해 궤적 반경을 실제 반경(45.0)과 동일하게 맞춤!
REAL_RADIUS  = 45.0            
PATH_RADIUS  = 45.0            
TOTAL_LAYERS = 3
...
def get_pure_blended_orientation(target_pos, center, tool_length=0.0):
    ...
    z_axis_final = normalize((down_z_axis * 1.0) + (pure_normal * 1.0))
...
def generate_force_dome_drl(center, real_radius, path_radius, total_layers, tool_length):
    ...
    # 순응 제어 ON (기준을 Tool(1)로 맞춰서 툴 방향이 부드러워짐)
    ...
    drl_code += '# 💡 힘 제어를 끄지 않고 그대로 유지한 채로 부드러운 돔 스캔 진입!\n'

    # 층별 회전 로직 (path_radius = 48.0 기준)
