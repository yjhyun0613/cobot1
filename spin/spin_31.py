...
# =================================================================
# [2] 기하학 연산 함수 (2:1 비율 덜 기울어짐 + 짐벌락 방어)
# =================================================================
def rot_to_zyz(R):
...
def get_dome_orientation(target_pos, center, tool_length=0.0):
    ...
    Z_t_pure = -normal # 돔 중심을 향함
    Z_t_down = np.array([0.0, 0.0, -1.0]) # 완벽한 수직 바닥 방향
    
    # 💡 [핵심] 유저 아이디어 적용: 수직(2) : 중심(1) 비율로 블렌딩!
    # 이렇게 하면 툴이 너무 눕지 않고 아래를 적당히 보면서 돕니다.
...
def generate_scan_drl(center, radius, total_layers, tool_length):
    ...
    for i in range(1, total_layers + 1):
        ...
        # 💡 [핵심] 층이 바뀔 때마다 이전 각도 기억을 지워서 -360도로 감기는 현상 원천 차단
        last_rx = None
        last_rz = None
    ...
    # 마지막 복귀도 블렌딩 로직을 통과한 수직 자세로 깔끔하게!
    a_rx_raw, a_ry, a_rz_raw, a_pos = get_dome_orientation(np.array([cx, cy, approach_z]), true_center, tool_length)
    drl_code += f"p_approach = posx({a_pos[0]:.2f}, {a_pos[1]:.2f}, {a_pos[2]:.2f}, {a_rx_raw:.2f}, {a_ry:.2f}, {a_rz_raw:.2f})\n"
...
class ScanRunnerNode(Node):
    ...
    def run(self):
        ...
        print("\n🚀 [Phase 2] 스캐닝 DRL 구동 시작 (2:1 비율 하이브리드 자세)...")
