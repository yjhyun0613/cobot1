    ...
    try:
        ...
        while rclpy.ok():
            ...
            if node.trigger_scan:
                ...
                task_compliance_ctrl([300, 300, 100, 300, 300, 300], 1)
                time.sleep(0.5)
                ...
                # ==========================================================
                # 💡 [긴급 안전 조치] 충돌 방지를 위한 수직 상승 (Retract)
                # ==========================================================
                print("\n⬆️ 돔 파고듦 방지: 안전 높이로 수직 상승합니다...")
                safe_z = CENTER_Z + R + 100.0  # 돔 꼭대기보다 100mm 위
                p_safe_up = [CENTER_X, CENTER_Y, safe_z, rx_top, ry_top, rz_top]
                movel(p_safe_up, vel=50.0, acc=100.0)
                time.sleep(1.0) # 안전하게 올라올 때까지 확실히 대기
                ...
                # 안전 높이에서 옆으로 이동하면서 내려오므로 돔과 부딪히지 않음
