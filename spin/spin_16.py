...
RADIUS       = 48.0            
TOTAL_LAYERS = 10
CENTER       = np.array([424.15, 66.45, 25.50]) # [X, Y, Z]

SAFE_Z_HEIGHT = 200.0  # 대기 위치의 안전 Z 높이

# 장착된 툴의 길이 (단위: mm, 3.5cm = 35.0mm)
TOOL_LENGTH  = 30    
...
def get_pure_blended_orientation(target_pos, center, tool_length=0.0):
    """자세 계산 및 툴 길이를 반영한 최종 그리퍼 위치 반환"""
    ...
    # 로봇이 바라보는 Z축의 반대 방향으로 툴 길이만큼 후퇴 (Offset)
...
# =================================================================
def generate_dome_scan_drl(center, radius, total_layers, tool_length):
    cx, cy, cz = center

    # 1. 첫 번째 층(Start) 위치 계산
    ...
    # 보정된 좌표(s1_pos) 받아오기
    s1_rx, s1_ry, _, s1_pos = get_pure_blended_orientation(start_1_xyz, center, tool_length)
    s1_rz = -180.0

    p_top = [s1_pos[0], s1_pos[1], SAFE_Z_HEIGHT, s1_rx, s1_ry, s1_rz]
    p_start_1 = [s1_pos[0], s1_pos[1], s1_pos[2], s1_rx, s1_ry, s1_rz]

    # 2. DRL 코드 뼈대 생성
    drl_code = "set_singularity_handling(1)\n"
    drl_code += "stiff = [100, 100, 100, 100, 100, 100]\n"
    ...
    try:
        ...
        while rclpy.ok():
            ...
            if node.trigger_scan:
                ...
                # 경로 생성 시 TOOL_LENGTH 파라미터 전달
                p_top, p_start, drl_code = generate_dome_scan_drl(CENTER, RADIUS, TOTAL_LAYERS, TOOL_LENGTH)
