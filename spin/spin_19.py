...
PATH_RADIUS  = 45.0            
TOTAL_LAYERS = 3
CENTER       = np.array([427.15, 75.45, 25.50]) # [X, Y, Z]
...
def generate_force_dome_drl(center, real_radius, path_radius, total_layers, tool_length):
    ...
    # 💡 [추가됨] 로봇이 자체적으로 가장 먼저 초기 조인트 위치로 가도록 설정
    drl_code += '# 1. 초기 대기 위치(원점)로 이동\n'
    drl_code += 'p_ready = posj(0.0, 0.0, 90.0, 0.0, 90.0, 0.0)\n'
    drl_code += 'movej(p_ready, v=100.0, a=100.0)\n\n'
    
    # 2. 돔 위쪽 접근 지점(Z=200을 거쳐서) 이동
    ...
    drl_code += 'fd_zero = [0.0, 0.0, zero_tz-6, 0.0, 0.0, 0.0]\n'
...
def main(args=None):
    ...
    try:
        ...
        # 💡 [핵심] 로봇 제어 로직을 전담할 백그라운드 작업 쓰레드
        def robot_task_worker():
            while rclpy.ok():
                if node.trigger_scan:
                    ...
                    # 에러 나던 Python 이동 코드는 지우고, 바로 DRL만 쏩니다!
                    print("\n🚀 제어기로 DRL 전송 중 (초기 대기 위치 이동부터 자동으로 시작)...")
        ...
        task_thread = threading.Thread(target=robot_task_worker, daemon=True)
        task_thread.start()
