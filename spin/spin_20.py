...
PATH_RADIUS  = 45.0            
TOTAL_LAYERS = 3
CENTER       = np.array([424.15, 75.45, 25.50]) # [X, Y, Z]
...
# =================================================================
# [3] 융합된 DRL 경로 생성기 (동적 오프셋 dz 적용됨)
# =================================================================
def generate_force_dome_drl(center, real_radius, path_radius, total_layers, tool_length):
    ...
    drl_code += f'expected_top_z = {cz + real_radius:.2f}\n'
    drl_code += 'dz = pos_touch[2] - expected_top_z\n\n'

    drl_code += 'release_force()\n'
    drl_code += 'fd_zero = [0.0, 0.0, zero_tz, 0.0, 0.0, 0.0]\n'
    drl_code += 'fdir_z = [0, 0, 1, 0, 0, 0]\n'
    drl_code += 'set_desired_force(fd_zero, fdir_z, 0) \n\n'
    ...
    for i in range(1, total_layers + 1):
        ...
        drl_code += f"s_z = {s_pos[2]:.2f} + dz\n"
        drl_code += f"v_z = {v_pos[2]:.2f} + dz\n"
        drl_code += f"t_z = {t_pos[2]:.2f} + dz\n"
        
        drl_code += f"p_start_{i} = posx({s_pos[0]:.2f}, {s_pos[1]:.2f}, s_z, {sr_x:.2f}, {sr_y:.2f}, {sr_z:.2f})\n"
        drl_code += f"p_via_{i}   = posx({v_pos[0]:.2f}, {v_pos[1]:.2f}, v_z, {vr_x:.2f}, {vr_y:.2f}, {vr_z:.2f})\n"
        drl_code += f"p_tgt_{i}   = posx({t_pos[0]:.2f}, {t_pos[1]:.2f}, t_z, {tr_x:.2f}, {tr_y:.2f}, {tr_z:.2f})\n"
...
# =================================================================
# [5] 메인 제어 흐름 (Single-Thread with spin_once)
# =================================================================
def main(args=None):
    ...
    try:
        ...
        # 💡 하나의 메인 루프 안에서 비동기와 순차 동작을 모두 안전하게 처리합니다.
        while rclpy.ok():
            rclpy.spin_once(node, timeout_sec=0.1) # 콜백 수신 대기
            ...
            if node.trigger_scan:
                ...
                print("\n🚀 제어기로 DRL 전송 중 (동적 오프셋 적용됨)...")
                node.send_drl_code(drl_code)
                time.sleep(1.0)
                
                print("⏳ 돔 꼭대기를 찾는 중입니다...")
                
                # 대기 루프 안에서도 spin_once를 돌려 통신이 먹통되지 않게 합니다.
                while rclpy.ok():
