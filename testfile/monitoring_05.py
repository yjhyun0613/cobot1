...
class TrajectoryMonitorNode(Node):
    def __init__(self):
        ...
        # 💡 팀원용 검사 결과(PASS/FAIL) 발행 퍼블리셔 생성
        self.result_pub = self.create_publisher(String, '/inspection_result', 10)
        
        self.get_logger().info('📡 실시간 궤적 모니터링 및 결과 발행 대기 중...')
    ...
    def state_callback(self, msg):
        if msg.data == True and not self.is_checking:
            ...
            self.get_logger().info('🔴 스캐닝 종료')
            self.is_checking = False
            
            # 💡 종료 시점까지 에러가 한 번도 안 났다면 PASS 발행
            if not self.error_detected:
                result_msg = String()
                result_msg.data = 'PASS'
                self.result_pub.publish(result_msg)
                self.get_logger().info('✅ 완벽합니다! 최종 검사 결과: [PASS] 발행 완료')
    ...
    def joint_callback(self, msg):
        ...
        try:
            ...
            if min_distance > self.ERROR_THRESHOLD:
                ...
                # 💡 에러 감지 즉시 FAIL 발행!
                result_msg = String()
                result_msg.data = 'FAIL'
                self.result_pub.publish(result_msg)
                self.get_logger().info('❌ 불량 감지! 검사 결과: [FAIL] 발행 완료')
