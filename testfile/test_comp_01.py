...
class DrlRunnerNode(Node):
    ...
    def publish_state(self, state: bool):
        ...
        self.get_logger().info("🟢 상태 신호 ON (바닥 훑기 기록 시작!)" if state else "🔴 상태 신호 OFF (검사 및 리포트 생성)")
...
p_start = posx(367.16, 70.33, 100.0, 0.0, 180.0, 90.0)
...
# 8.5 툴 무게 버티기 (영점 보상 켜기)
fd_zero = [0.0, 0.0, zero_z-6, 0.0, 0.0, 0.0]
fdir_z = [0, 0, 1, 0, 0, 0]
set_desired_force(fd_zero, fdir_z, 0) 

# 9. 툴 무게를 스스로 버티는 상태에서 전진!
target_z = floor_z - 4.0 
p_scrape = posx(367.16, 70.33, target_z, 20.0, 180.0, 90.0)
movel(p_scrape, v=10.0, a=40.0)

# 9. 툴 무게를 스스로 버티는 상태에서 전진!
p_scrape = posx(476.41, 70.33, target_z, 20.0, 180.0, 90.0)
movel(p_scrape, v=10.0, a=40.0)
...
p_end = posx(367.16, 70.33, 100.0, 0.0, 180.0, 0.0)
...
"""
        ...
        self.get_logger().info('✅ 전송 완료! 로봇이 바닥 탐색을 시작합니다.')
...
def main(args=None):
    ...
    try:
        # 💡 get_current_posx 함수 추가 임포트
        from DSR_ROBOT2 import (
            set_robot_mode, ROBOT_MODE_AUTONOMOUS, ROBOT_MODE_MANUAL,
            get_robot_mode, set_tool, set_tcp, get_tcp, get_current_posx
        )
        ...
        # 💡 2. [핵심 로직] 로봇이 바닥에 닿고 앞으로 가기 시작하는 순간 포착!
        while rclpy.ok():
            current_pos = get_current_posx()[0]
            current_x = current_pos[0]
            
            # 시작 X좌표가 367.16이므로, 375.0을 넘으면 훑기(전진)를 시작한 것!
            if current_x > 375.0:
                node.publish_state(True) # 🟢 이때부터 기록 시작
                break
            time.sleep(0.05) # 0.05초마다 감시
            
        # 3. 전진 훑기 동작(X=476.41까지)이 완료될 때까지 넉넉히 대기
        # (거리 약 110mm / 속도 20 이므로 약 5~6초 소요됨)
        time.sleep(7.0)              
