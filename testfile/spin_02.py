...
class ScanRunnerNode(Node):
    ...
    def publish_state(self, state: bool):
        ...
        status_text = "🟢 ON (데이터 기록/검사 시작)" if state else "🔴 OFF (데이터 기록/검사 종료)"
...
def main(args=None):
    ...
    try:
        ...
        # ==========================================================
        print("\n🚀 초기 위치(꼭대기)로 이동 중... (기록 안됨)")
        # 💡 p_ready의 6번째 값이 0.0입니다. (6번 조인트 0도)
...
# ==========================================================
# 💡 [핵심 수정] 종료 후 6번 조인트 꼬임을 풀기 위해 movej 사용
# ==========================================================
p_ready_drl = posj(0.0, 0.0, 90.0, 0.0, 90.0, 0.0)
movej(p_ready_drl, v=60, a=100)
"""
        ...
        # 💡 폴링(Polling) 로직: 로봇이 스캐닝을 완전히 마칠 때까지 대기
        while True:
            state = get_robot_state()
            # 1: ROBOT_STATE_IDLE (모든 움직임이 끝나고 대기 중)
            if state == 1: 
                break
            time.sleep(0.2)

        # ==========================================================
        # [Step 4] 스캐닝 완료! 신호 OFF
        # ==========================================================
        node.publish_state(False)
        time.sleep(1.0) # 다른 노드들이 numpy 파일을 안전하게 저장할 시간 확보

        # ==========================================================
        # 💡 [핵심 수정] 파이썬에서도 안전하게 초기 관절 상태로 복귀
        # ==========================================================
        print("\n작업 완료. 6번 조인트 꼬임을 풀고 대기 자세로 복귀합니다.")
        movej(p_ready, vel=60.0, acc=100.0)
