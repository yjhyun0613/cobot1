...
def initialize_robot():
    # 💡 set_ref_coord를 추가로 불러옵니다.
    from DSR_ROBOT2 import (
        set_tool, set_tcp, set_robot_mode, 
        set_ref_coord, DR_BASE,  # 좌표계 설정을 위한 상수
        ROBOT_MODE_MANUAL, ROBOT_MODE_AUTONOMOUS
    )
    
    print("🔄 좌표계 및 로봇 설정 초기화 중...")
    set_robot_mode(ROBOT_MODE_MANUAL)
    
    # 1. 툴과 TCP를 초기화 (Flange 기준)
    set_tool("")
    set_tcp("")
    
    # 2. 💡 핵심: 참조 좌표계를 로봇 베이스(Base)로 강제 고정
    # 이전 프로그램이 남긴 좌표계 설정을 무시하고 초기 상태로 돌립니다.
    set_ref_coord(DR_BASE) 
    
    set_robot_mode(ROBOT_MODE_AUTONOMOUS)
    time.sleep(1)
    print("✅ 좌표계가 로봇 베이스(DR_BASE)로 설정되었습니다.")
...
def draw_triple_circles(node):
    ...
    # 💡 반지름 리스트 (50mm, 40mm, 30mm 총 3개)
    radii = [50.0, 40.0, 30.0]
