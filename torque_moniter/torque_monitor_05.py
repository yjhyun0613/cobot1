...
from sensor_msgs.msg import JointState # 💡 조인트 상태 메시지 추가
from dsr_msgs2.srv import GetExternalTorque

# =========================================================
# 🧮 순운동학 (Forward Kinematics) 위치 계산 함수
# =========================================================
...
def forward_kinematics(q):
    ...
    return pos_meters * 1000.0 # mm 단위로 변환 반환
...
class InspectionMasterNode(Node):
    def __init__(self):
        ...
        # 💡 [핵심] PosX 서비스 콜을 제거하고 JointState 구독으로 변경
    ...
    def tq_cb(self, future):
        try:
            ...
            # 💡 [디버깅 추가] 터미널에 실시간으로 좌표와 상태 출력
            import sys
            sys.stdout.write(
                f"\r🔍 [기록 중] X:{self.latest_pos[0]:6.1f} Y:{self.latest_pos[1]:6.1f} Z:{self.latest_pos[2]:6.1f} | "
                f"외력: {ext_tq:5.2f} Nm | 오차율: {self.max_error_rate:5.2f}%   "
            )
            sys.stdout.flush()
            ...
            if is_crack and not self.crack_active:
                ...
                self.post_count = 0
                print(f"\n💥 [크랙 감지!] 외력 {ext_tq:.2f} Nm (Threshold: {CRACK_THRESHOLD}) - 여유 데이터를 모읍니다.")
    ...
    def run_hemiscan_analysis(self):
        ...
        try:
            ...
            # 💡 [디버깅 추가] 분석 완료 시 이탈 거리 알림
            print(f"\n📊 [분석 완료] 실제 궤적 이탈 거리: {e_max:.3f} mm -> 오차율 계산: {error_rate:.3f}%")
    ...
    def generate_and_upload_report(self):
        self.get_logger().info('📝 FAIL 리포트를 생성하고 업로드합니다 (백그라운드)...')
