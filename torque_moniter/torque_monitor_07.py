...
class InspectionMasterNode(Node):
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().info('✅ Firebase 연결 완료! (리얼타임 DB 기록 추가됨)')
        except: pass
    ...
    def run_hemiscan_analysis(self):
        ...
        if len(t_n) < 4: return
        try:
            spline = CubicSpline(t_n, p_n); p_i = spline(t_a)
            dist = np.linalg.norm(p_a - p_i, axis=1)
            e_max = np.max(dist); rate = (e_max / RADIUS) * 100
            if rate > self.max_error_rate: self.max_error_rate, self.max_e_max = rate, e_max
        except: pass

    # =========================================================
    # 📄 [수정] FAIL 리포트 업로드 및 실시간 DB(Realtime DB) 기록
    # =========================================================
    def generate_and_upload_report(self):
        ...
        """
        try:
            # 1. Firebase Storage에 HTML 파일 업로드
            blob = self.bucket.blob(f'inspection_reports/{filename}')
            blob.upload_from_string(html_content, content_type='text/html')
            blob.make_public()
            
            # 2. 💡 [추가] Realtime Database "robot/defect" 경로에 기록
            db.reference("robot/defect").push({
                ...
                "storageUrl": blob.public_url # 업로드된 파일의 공개 링크
            })
