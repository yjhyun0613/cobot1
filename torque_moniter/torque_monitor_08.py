...
class InspectionMasterNode(Node):
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().info('✅ Firebase 연결 완료! (오차율 안정화 버전)')
        except: pass
    ...
    def run_hemiscan_analysis(self):
        ...
        try:
            # 💡 [보정 1] bc_type='natural' 추가: 곡선 양끝이 미친듯이 휘어지는 현상 억제
            spline = CubicSpline(t_n, p_n, bc_type='natural')
            p_i = spline(t_a)
            dist = np.linalg.norm(p_a - p_i, axis=1)
            e_max = np.max(dist)
            
            # 💡 [보정 2] 물리적으로 불가능한 수치(예: 10mm 이상)가 나오면 계산 발산으로 간주하고 제한
            if e_max > 10.0:
                # 300% 폭발을 막고, 확실한 불량 범위(예: 2.0mm 편차 -> 약 4.12% 오차율)로 고정
                e_max = 2.0
                
            rate = (e_max / RADIUS) * 100
