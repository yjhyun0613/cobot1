...
import os
import datetime
import requests  # 💡 파이어베이스 업로드를 위한 모듈 추가
import json      # 💡 파이어베이스 업로드를 위한 모듈 추가
...
class PCTestRecorderNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('📡 PC 테스트용 기록 대기 중... (npy + html 저장 및 Firebase 자동 업로드)')
    ...
    def save_data_and_plot(self):
        ...
        # ==========================================================
        # 💡 [추가] 파이어베이스에 기준 궤적(Baseline) 자동 업로드
        # ==========================================================
        self.get_logger().info("☁️ 웹 모니터링용 Baseline 자동 업로드를 시작합니다...")
        try:
            # 웹 렌더링 최적화를 위해 데이터 1/2로 줄이기 (버벅임 방지)
            pts_reduced = pts[::2] 
            
            # x, y, z 리스트 형태로 변환 (Plotly가 읽을 수 있는 형태)
            data = {
            ...
            response = requests.put(url, data=json.dumps(data), timeout=5.0)
            
            if response.status_code == 200:
                self.get_logger().info("✅ Baseline 자동 업로드 완료! (웹 대시보드를 새로고침 해보세요)")
            else:
                self.get_logger().error(f"❌ 업로드 실패: {response.text}")
                
        except Exception as e:
            self.get_logger().error(f"🚨 파이어베이스 업로드 중 에러 발생: {e}")
        # ==========================================================
