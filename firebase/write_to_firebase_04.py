...
# =========================================================
# 1. 수학 계산 함수 (정기구학 및 오일러 각도 변환) 추가
# =========================================================
    ...
    def robot_state_callback(self, msg):
        ...
        if msg.position and len(msg.position) >= 6:
            try:
                ...
                # 3. Firebase DB에 조인트 및 TCP 데이터 쓰기 (업데이트)
                self.ref.update({
                    # 조인트 정보 (라디안 단위, 소수점 4자리)
                    ...
                    # TCP(끝단) 위치 정보 (mm 단위, 소수점 2자리)
                    'tcp_x': round(x, 2),
                    'tcp_y': round(y, 2),
                    'tcp_z': round(z, 2),
                    
                    # TCP(끝단) 자세 정보 (Degree 단위, 소수점 2자리)
                    'tcp_a': round(a, 2),
                    'tcp_b': round(b, 2),
                    'tcp_c': round(c, 2),
                ...
                # 콘솔 창이 너무 번잡해지는 것을 막기 위해 X, Y, Z만 간단히 로그 출력
                self.get_logger().info(f"Firebase 업데이트 완료 (X: {x:.1f}, Y: {y:.1f}, Z: {z:.1f})")
                
            except KeyError:
                # 관절 이름 매핑에 실패한 메시지(예: 그리퍼 토픽만 들어온 경우)는 무시합니다.
                pass
            except Exception as e:
