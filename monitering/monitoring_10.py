...
class TrajectoryMonitorNode(Node):
    ...
    def save_and_upload_report(self, result, rate, max_err, mean_err):
        ...
        # 💡 [핵심 1] 반투명 40% 히트맵 면 (Mesh) 생성
        # 로봇이 훑고 지나간 실제 점들을 이어서 면을 만들고, 에러값(dists)에 따라 색상을 칠합니다.
        fig.add_trace(go.Mesh3d(
            x=pts[:, 0], 
            y=pts[:, 1], 
            z=pts[:, 2],
            intensity=dists,           # 각 꼭짓점(점)의 오차값 입력
            colorscale='Jet',          # 파란색(0) -> 노란색 -> 빨간색(5)
            cmin=0,                    # 정상 구간
            cmax=self.ERROR_THRESHOLD, # 에러 기준치 (이 이상은 무조건 진한 빨간색)
            opacity=0.4,               # 40% 반투명 유리 느낌
            alphahull=-1,              # 점들을 삼각망(Delaunay)으로 감싸서 돔 표면 생성
            name='Error Heatmap Surface',
            colorbar=dict(title="오차 (mm)", thickness=20)
        ))

        # 💡 [보조] 기준 궤적(정답지)은 유리를 통해 비치도록 점선으로 안쪽에 추가
        ...
        # 💡 [보조] 뼈대 역할을 할 실제 궤적 라인 (면과 함께 보여주면 더 입체적입니다)
        fig.add_trace(go.Scatter3d(
        ...
        fig.update_layout(
            ...
            scene=dict(aspectmode='data'), # 실제 비율 유지
