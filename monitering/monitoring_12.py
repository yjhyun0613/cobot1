...
class TrajectoryMonitorNode(Node):
    def __init__(self):
        ...
        # 💡 [핵심 변경] 점(Point) 데이터를 불러와서 가상의 3D 면(Mesh)으로 변환
        try:
    ...
    def joint_callback(self, msg):
        ...
        try:
            ...
            # 💡 [핵심 변경] 점과 가장 가까운 "표면(Mesh Face)"까지의 최단 직교 거리 계산
    ...
    def save_and_upload_report(self, result, rate, max_err, mean_err):
        ...
        # 💡 [핵심 변경] 기준 돔 형상을 점(Point)이 아닌 매끄러운 3D 껍질(Mesh)로 렌더링
        fig.add_trace(go.Mesh3d(
            x=self.baseline_mesh.vertices[:, 0], 
            y=self.baseline_mesh.vertices[:, 1], 
            z=self.baseline_mesh.vertices[:, 2],
            i=self.baseline_mesh.faces[:, 0], # 삼각형 꼭짓점 1
            j=self.baseline_mesh.faces[:, 1], # 삼각형 꼭짓점 2
            k=self.baseline_mesh.faces[:, 2], # 삼각형 꼭짓점 3
            color='lightgray',
            opacity=0.3, # 반투명하게 설정하여 에러 점들이 잘 보이도록 함
